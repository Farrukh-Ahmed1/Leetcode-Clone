...
import { AiFillLike, AiFillDislike, AiOutlineLoading, AiOutlineLoading3Quarters, AiFillStar } from "react-icons/ai";
...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (disliked) {
          ...
          transaction.update(userRef, {
            likedProblems: userDoc.data().likedProblems.filter((id: string) => id !== problem.id),
            dislikedProblems: Array.isArray(userDoc.data().dislikedProblems)
              ? [...userDoc.data().dislikedProblems, problem.id]
              : [problem.id],
          });
          transaction.update(problemRef, {
            likes: problemDoc.data().likes - 1,
  ...
  return (
    <div className="bg-dark-layer-1">
      {/* TAB */}
      <div
        className="flex h-11 w-full items-center pt-2 bg-dark-layer-2 text-white 
      overflow-x-hidden"
      >
        <div
          className={
          ...
          <div className="w-full">
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                ...
                {(solved || _solved) && (
                  <div
                  className="rounded p-[3px] ml-4 text-lg transition-colors 
              duration-200 text-green-s text-dark-green-s"
                >
                  <BsCheck2Circle />
                </div>
                )}
                <div
                  className="flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 
  ...
  useEffect(() => {
    ...
    const getCurrentProblem = async () => {
      ...
      if (docSnap.exists()) {
        ...
        setLoading(false);
        setCurrentProblem({ ...problem, id: docSnap.id } as DBProblem);
  ...
  useEffect(() => {
		const getUsersDataOnProblem = async () => {
			...
			if (userSnap.exists()) {
				...
				setData({
					liked: likedProblems && likedProblems.includes(problemId),
          disliked: dislikedProblems && dislikedProblems.includes(problemId),
          starred: starredProblems && starredProblems.includes(problemId),
          solved: solvedProblems && solvedProblems.includes(problemId)
				});
			}
		};
